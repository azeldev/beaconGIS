...
class ChangeDetector:
    ...
    # Python packages required at plugin runtime. The pip name differs
    # from the import name for some (cv2 → opencv-python, PIL → Pillow).
    # On Windows we prefer onnxruntime-directml so users get GPU
    # acceleration on any DX12 card without installing CUDA Toolkit.
    @staticmethod
    def _required_dependencies():
        import sys
        is_win = sys.platform == 'win32'
        return [
            # (import_name, pip_install_name, friendly_label)
            ('onnxruntime',
    ...
    def _check_dependencies(self):
        """Returns (ok, list_of_missing_pip_install_names, friendly_labels)."""
        missing_pip = []
        missing_labels = []
        for import_name, pip_name, label in self._required_dependencies():
            try:
                __import__(import_name)
            except ImportError:
                missing_pip.append(pip_name)
                missing_labels.append(label)
        return (len(missing_pip) == 0, missing_pip, missing_labels)

    def _auto_install_dependencies(self, missing_pip):
        """Run pip install for the given packages via the QGIS-bundled
        Python. Returns True on success, False on failure or user-cancel.

        Tries unprivileged install first; falls back to --user. Both
        return paths are wrapped in clear dialogs so a non-CLI user
        understands what happened."""
        import sys
        import subprocess

        pip_cmd_display = (f'"{sys.executable}" -m pip install '
                           + ' '.join(missing_pip))

        msg = QMessageBox(self.iface.mainWindow())
        msg.setIcon(QMessageBox.Question)
        msg.setWindowTitle("Install missing dependencies?")
        msg.setText("<b>beaconGIS</b> needs the following Python packages:")
        msg.setInformativeText(
            "  • " + "<br>  • ".join(missing_pip) + "<br><br>"
            "<b>Auto-install them now?</b><br><br>"
            "Install runs from inside QGIS via pip; takes 1–3 minutes. "
            "QGIS will be briefly unresponsive while pip works. "
            "Restart QGIS afterwards to load the new libraries.")
        ...
        if reply == QMessageBox.Help:
            QMessageBox.information(
                self.iface.mainWindow(), "Manual install",
                "Open the <b>OSGeo4W Shell</b> (Windows Start menu) — "
                "not regular cmd — then paste:<br><br>"
                f"<pre>{pip_cmd_display}</pre><br>"
        ...
        self.iface.messageBar().pushMessage(
            ...
            f"QGIS may be unresponsive for 1–3 min...",
            level=Qgis.Info, duration=0)
        # Force the message bar to repaint before the blocking pip call.
        ...
        try:
            # Try a system-site install first (works inside OSGeo4W where
            # the user owns the Python tree). Fall back to --user on
            # permission errors so it still lands somewhere importable.
            result = _run_pip([])
            if result.returncode != 0 and (
                    'permission' in result.stderr.lower()
                    or 'access is denied' in result.stderr.lower()
                    or 'externally-managed' in result.stderr.lower()):
            ...
            if result.returncode == 0:
                QMessageBox.information(
                    self.iface.mainWindow(), "Install complete",
                    "Dependencies installed successfully.<br><br>"
                    "<b>Please restart QGIS</b> so the plugin can load "
                    "the new libraries.")
                return True

            tail = (result.stderr or result.stdout or '')[-1000:]
            QMessageBox.critical(
                self.iface.mainWindow(), "Install failed",
                f"pip exited with code {result.returncode}.<br><br>"
                f"Last output:<br><pre>{tail}</pre><br>"
                f"Try manually from the OSGeo4W shell:<br>"
                f"<pre>{pip_cmd_display}</pre>")
            return False
        except subprocess.TimeoutExpired:
            self.iface.messageBar().clearWidgets()
            QMessageBox.critical(
                self.iface.mainWindow(), "Install timeout",
                "pip install timed out after 10 minutes. "
                "Try manually from the OSGeo4W shell:<br>"
                f"<pre>{pip_cmd_display}</pre>")
            return False
        except Exception as e:
            self.iface.messageBar().clearWidgets()
            QMessageBox.critical(
                self.iface.mainWindow(), "Install error",
    ...
    def process_damage_detection(self):
        # Dependency check first — offer auto-install if anything is missing.
        ok, missing_pip, missing_labels = self._check_dependencies()
        if not ok:
            installed = self._auto_install_dependencies(missing_pip)
            # We always return here — even on success, the new packages
            # are loaded fresh after the QGIS restart we ask for.
            return
